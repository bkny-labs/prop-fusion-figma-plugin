...
const ConfigForm = forwardRef((props, ref) => {
  ...
  const typescriptFrameworks = ['react', 'angular', 'vue'];
  const styleFrameworks = ['react', 'angular', 'vue', 'webComponents'];
  ...
  useEffect(() => {
    if (!typescriptFrameworks.includes(framework)) {
      setTypescript('');
      setTypescriptChecked(false);
    }
  }, [framework]);
  ...
  const frameworks = [
    ...
    { value: 'webComponents', label: 'Web Components', Icon: SiWebcomponentsdotorg, color: '#29ABE2' },
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      {typescriptFrameworks.includes(framework) && (
      <div>
        <h3>Typescript</h3>
          <div className="">
      ...
      {styleFrameworks.includes(framework) && (
      <div>
        <h3>Styles</h3>
        <label>
